...
- 유형 필터 6개 (전체/초/중/고/특수/각종) — 헤더 버튼 + 모바일 칩
- 구/군 드롭다운 필터
- 정렬: 이름순, 학생수, 학급수, 학급당 학생수 (각 오름/내림)
- **전체 필터** 상태에서는 특수·각종학교도 초·중·고와 함께 노출됨 (총 252개).
...
- 초등학교 선택 시 통학구역 폴리곤 표시 (녹색 실선, fillOpacity: 0.2), 중학교 선택 시 중학구/학교군 폴리곤 표시 (파란 실선, fillOpacity: 0.15). 고등학교는 자동 표시 안 함 (사용자 요청)
...
- 팝업 닫으면 통학구역 진한 강조는 해제, 사이드바 선택 상태는 유지 (전체 학구 토글 레이어는 영향 없음)
