...
scripts/
  import_excel.py         — 엑셀 5개(xlsx) → ulsan_schools.json 통합 변환 (openpyxl)
  migrate_xls_to_xlsx.py  — 일회성 마이그레이션: 울산학교주소위도경도.xls→.xlsx + 좌표 보정값 주입 (실행 완료, 참고용 보관)
...
| 울산학교주소위도경도.xlsx | 주소, 위도, 경도 ⚠️ 위도/경도는 공식 원본이 아니라 245개 중 210건이 보정된 값. 원본은 git 히스토리(커밋 a66f2ea의 .xls) 참고. 보정 경위는 커밋 742c105, d528a44 참고. 새 학교 추가 시에도 좌표가 정확한지 확인 필요 |
